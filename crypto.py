...
def encodeText():  
    ...
    img_name = cv2.resize(img, (500, 500))  
...
def decodeText():  
    ...
    img = cv2.imread(img_name, cv2.IMREAD_UNCHANGED)  # reading the image using the imread() function  
  
    print("The Steganographic image is as follow: ")  
    img_name = cv2.resize(img, (500, 500))    # resizing the actual image as per the needs  
    cv2.imshow("img_name", img)  # displaying the Steganographic image  
    print(img)
